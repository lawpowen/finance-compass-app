...
import '../../core/utils/transaction_cash_flow.dart';
...
class _TransactionsScreenState extends ConsumerState<TransactionsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final transferNetEffect = transferNetEffectTotal(
      summaryTransactions,
      accountId: effectiveAccountId,
      outgoingAmountInBase: repository.transactionAmountInBase,
      incomingAmountInBase: repository.transferIncomingAmountInBase,
    );
    final netCashFlowWithTransfers = netCashFlow + transferNetEffect;
      ...
      children: [
                  ...
                  children: [
                    ...
                    FinanceMetricGrid(
                      ...
                      children: [
                        ...
                        FinanceMetricCard(
                          label: '净现金流（含转账）',
                          value: formatMoney(netCashFlowWithTransfers),
                          color: netCashFlowWithTransfers >= 0
                              ? FinanceColors.income
                              : FinanceColors.expense,
                        ),
                      ],
                    ...
                    Text(
                      effectiveAccountId == null
                          ? '查看全部账户时，内部转账的净影响为 ${formatMoney(0)}'
                          : '所选账户的转账净影响：${formatMoney(transferNetEffect)}',
        ...
        Positioned(
          ...
          child: Column(
            children: [
              CompassActionBubble(
                heroTag: 'transactions_quick',
                icon: Icons.bolt_rounded,
                onPressed: () => _showQuickTemplatePicker(context),
              ),
              const SizedBox(height: 12),
              CompassActionBubble(
                heroTag: 'transactions_add',
  ...
  // Kept for the template-management screen that will reuse this action sheet.
  ...
  // Kept for the recurring-rule management screen.
  // ignore: unused_element
  ...
  Future<void> _showQuickTemplatePicker(BuildContext context) async {
    final template = await showModalBottomSheet<TransactionTemplate>(
      context: context,
      showDragHandle: true,
      builder: (context) => SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('快速模板', style: Theme.of(context).textTheme.titleLarge),
              const SizedBox(height: 12),
              if (widget.repository.transactionTemplates.isEmpty)
                const Padding(
                  padding: EdgeInsets.symmetric(vertical: 24),
                  child: Text('还没有模板，可从任意交易的操作菜单建立。'),
                )
              else
                ...widget.repository.transactionTemplates.map(
                  (item) => ListTile(
    ...
    switch (transaction.type) {
      ...
      case TransactionType.adjustment:
        return '${signedAmount > 0 ? '+' : ''}'
            '${formatMoney(signedAmount, currency: transaction.currency)}';
      case TransactionType.transfer:
