...
2. 使用 `apksigner verify --verbose --print-certs` 验证 APK 签名；确认包名 `com.financecompass.app`、版本 `0.8.0`、versionCode `41` 和 ARM64/目标 ABI。
...
5. 生成 `SHA256SUMS.txt`，重新计算三项哈希并逐项比对。
...
3. 创建非草稿、非预发布 GitHub Release，上传三个二进制产物和 `SHA256SUMS.txt`。
