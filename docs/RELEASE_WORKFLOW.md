...
4. 使用 `flutter test tool/export_release_data_test.dart --dart-define=SOURCE_DB=<sqlite-snapshot> --dart-define=OUTPUT_JSON=<output-json>` 从一致性快照生成完整 v3 JSON；工具通过 Flutter 运行器调用应用自身 Repository，并验证账户、分类、预算、交易、投资快照、快速模板、周期规则和 meta 结构。
