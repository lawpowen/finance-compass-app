...
同币种转账只使用一个权威金额：`amount` 同时代表来源扣款和目标入账，`to_amount` 必须保存为 `NULL`。`to_amount` 仅为未来跨币种转账保留。受影响的旧 Debug 版本可能由快速模板写入不可见的 `to_amount=0`，导致来源已扣款而目标未入账；一次性迁移 `zero_same_currency_transfer_amounts_v1` 会把非零、同币种、目标有效的转账归一为 `NULL`，并仅对 `actual`/`settled` 记录补回目标余额，`planned` 只归一字段而不影响余额。完整 JSON 导入会强制扫描同一异常，修复在替换数据与完整性检查的同一事务内完成，源 JSON 不会被覆盖。
