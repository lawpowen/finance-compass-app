...
| `FinanceCompass-SelfHost-Ubuntu-v0.9.0.zip` | 15,074,666 | `50da22052e2d01ab720cfa1347ba6e6a92f418b6bfa392afe02e27a953f42655` |
| `FinanceCompass-SelfHost-Windows-v0.9.0.zip` | 15,074,666 | `220b53acfc53cf8b127ad63be04ec22a6e6ec2da33f642df7b7cb48275c8fc7b` |
| `FinanceCompass-SelfHost-macOS-v0.9.0.zip` | 15,074,666 | `778a2d95803cf64520e8a2a5684601c90629b9df2ca0d610c9773be2026c427c` |
...
- 自托管：三个 ZIP 各含 52 个文件条目、预构建 `webroot`、runtime Dockerfile、Compose、Caddy 配置和平台启动脚本；条目全部使用 ZIP 标准 `/` 分隔符，每包内 WASM 哈希正确且无 Worker 源码/map/deps sidecar。
...
- 本机不承担 Docker 验收；自托管包改在 Tailscale homelab 的 Ubuntu、Docker 29.6.2 与 Docker Compose 5.3.1 环境运行。实测发现并修复 Windows ZIP 反斜杠条目与 Caddy 文件 capability 在最小权限容器下无法执行的问题，修复后按原发布 ZIP 解压、构建、启动和回环 HTTP 验证通过。iOS Safari/Android Chrome 的 HTTPS 安装、离线重开与系统存储回收仍需目标设备人工验收。
